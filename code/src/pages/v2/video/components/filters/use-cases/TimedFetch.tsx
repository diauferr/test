...
import { VideoNavBarContainer, VideoNavBarResults } from '../../../styles';
...
export const TimedFetch = ({
  ...
  const selecteds = data.filter((e) => e.checked);
  ...
  const fetchData = (query: string) => {
    const fetchId = ri;
    const url = `${process.env.REACT_APP_ENTITY_SEARCH}/video/filter/${segment}`;
    const options = {
      params: { query },
      headers: {
    ...
    <VideoNavBarResults>
      ...
      {!loading && !error ? (
        <>
          ...
          {data.length ? <div className="results">{results}</div> : null}
        </>
      ) : null}
    </VideoNavBarResults>
  );
